...
void LoadWeights(Weights& w, Tensor2dTok& tok_emb_table, std::ifstream& fs) {
  ...
  InitTensor(w.rms_att_w, fs);       // [kNLayers, kDim]
  InitTensor(w.attn_wq, fs);         // [kNLayers, kDim, kDim]
  InitTensor(w.attn_wk, fs);         // [kNLayers, kDim, kDim]
  InitTensor(w.attn_wv, fs);         // [kNLayers, kDim, kDim]
  InitTensor(w.attn_wo, fs);         // [kNLayers, kDim, kDim]
  InitTensor(w.rms_ffn_w, fs);       // [kNLayers, kDim]
  InitTensor(w.ffn_w1, fs);          // [kNLayers, kHiddenDim, kDim]
  InitTensor(w.ffn_w2, fs);          // [kNLayers, kDim, kHiddenDim]
  InitTensor(w.ffn_w3, fs);          // [kNLayers, kHiddenDim, kDim]
  InitTensor(w.rms_final, fs);       // [kDim]
  InitTensor(w.cos_table, fs);       // [kSeqLen, kDim / kNHeads / 2]
  InitTensor(w.sin_table, fs);       // [kSeqLen, kDim / kNHeads / 2]
}
